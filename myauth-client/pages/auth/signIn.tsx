...
const SignIn = ({ providers, csrfToken }: any) => {
    const [isCredentials, setIsCredentials] = React.useState(false);

    // better approach for checking providers available and displaying the option
    React.useEffect(() => {
        setIsCredentials(_.filter(providers, (provider: any) => provider.type === 'credentials').length > 0 );
    }, [providers]);
                ...
                <div className="bg-white py-8 px-4 shadow sm:rounded-lg sm:px-10">
                    {
                        ...
                        <form className="space-y-6" action="/api/auth/callback/credentials" method="POST">
                            ...
                            <div>
                                <button
                                    type="submit"
                                    className="w-full h-12 flex justify-center py-2 px-4 cursor-pointer border-4 border-solid border-gray-600 rounded shadow-sm text-md font-medium text-gray-900 bg-white focus:outline-none"
                                >
                                    Sign in
                                </button>
                            </div>
